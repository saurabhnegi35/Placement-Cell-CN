...
module.exports.createNewCompany = function (req, res) {
  ...
  CompanyDB.findOne(
    ...
    function (err, company) {
      if (err) {
        console.log(
          "Error in finding company inside createNewCompany :: ",
          err
        );
        return;
      }
...
module.exports.addStudentInterview = function (req, res) {
  ...
  CompanyDB.findById(req.params.id, function (err, Company) {
    if (err) {
      console.log(
        "Error in finding company inside addStudentInterview :: ",
        err
      );
      return;
    }
...
module.exports.assignInterviewToStudent = function (req, res) {
  ...
  InterviewDB.findOne(
    { student: req.body.student, company: req.body.company_id },
    function (err, inetrviewList) {
      if (err) {
        console.log(
          "Error in finding interview in assignInterviewToStudent in interview_controller :: ",
          err
        );
        return;
      }
      ...
      if (inetrviewList) {
        req.flash(
          "error",
          "Student Already scheduled Interview for Selected comapany"
        );
        console.log(
          "Interview is already schedule for this comapany this student..."
        );
        return res.redirect("back");
      }
...
module.exports.updateInterviewStatus = function (req, res) {
  ...
  InterviewDB.findByIdAndUpdate(
    ...
    function (err, updatedInterview) {
      ...
      req.flash("success", "Interview Status Updated Successfully");
      return res.redirect("back");
    }
  );
};
