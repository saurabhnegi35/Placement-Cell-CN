...
module.exports.createStduent = function (req, res) {
  ...
  StduentDB.findOne({ email: req.body.email }, function (err, student) {
    ...
    if (student) {
      req.flash("error", "Student is Already Exists");
...
module.exports.deleteStduent = function (req, res) {
  ...
  StduentDB.findById(req.params.id, function (err, student) {
    ...
    //delete student
    student.remove();
    req.flash("success", "Student and Interview was Deleted Successfully");
...
module.exports.viewStudent = function (req, res) {
  ...
  StduentDB.findById(req.params.id, function (err, student) {
    ...
    InterviewDB.find({ student: student.id })
      .populate("company")
      .exec(function (err, interviewList) {
        if (err) {
          console.log(
            "Error in finding interview in CompanyDB in side viewStudent ",
            err
          );
          return;
        }
...
module.exports.updateStudent = function (req, res) {
  StduentDB.findByIdAndUpdate(
    req.params.id,
    req.body,
    function (err, updatedStudent) {
      if (err) {
        console.log(
          "Error in updating  student in side updateStudent :: ",
          err
        );
        return;
      }
